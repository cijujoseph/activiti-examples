...
public class CustomJobRetryCmd implements Command<Object> {

  private static final Logger log = LoggerFactory.getLogger(CustomJobRetryCmd.class.getName());
  ...
  public Object execute(CommandContext commandContext)  {
    ...
    if (activity == null || activity.getFailedJobRetryTimeCycleValue() == null) {
      ...
      //Start "Fibonacci backoff retry" modifications
      int defaultMaxRetries = 10;
      //Set default if first time by checking if there is an exception message associated with the job in the table
      if (job.getExceptionMessage() == null) {
    	  job.setRetries(defaultMaxRetries);
      }
      int retriesLeft = job.getRetries() - 1;
      
      job.setRetries(retriesLeft);
      job.setLockOwner(null);
      job.setLockExpirationTime(null);

      if (job.getDuedate() == null) {
        // add wait time for failed async job
        job.setDuedate(calculateDueDate(commandContext, fib(defaultMaxRetries-retriesLeft)*processEngineConfig.getAsyncFailedJobWaitTime(), null));
      } else {
        // add default wait time for failed job
        job.setDuedate(calculateDueDate(commandContext, fib(defaultMaxRetries-retriesLeft)*processEngineConfig.getDefaultFailedJobWaitTime(), job.getDuedate()));
      }
      //End "Fibonacci backoff retry" modifications 
